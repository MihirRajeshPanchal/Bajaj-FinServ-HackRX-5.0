...
def add_bullet_text_to_slide(
    ...
    requests = [
        ...
        {"insertText": {"objectId": object_id, "text": bullet_points_text}},
        {
            "updateTextStyle": {
                "objectId": object_id,
...
def copy_presentation(drive_service, original_presentation_id: str, new_name: str):
    body = {"name": new_name}
    copied_file = (
        drive_service.files().copy(fileId=original_presentation_id, body=body).execute()
    )
    return copied_file
...
def export_presentation(
    drive_service, presentation_id, export_format="application/pdf"
):
    """Export a Google Slides presentation to a specified format and download it."""
    request = drive_service.files().export_media(
        fileId=presentation_id, mimeType=export_format
    )
