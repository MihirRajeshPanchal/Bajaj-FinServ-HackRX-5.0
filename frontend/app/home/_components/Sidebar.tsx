...
const SidebarItem: React.FC<SidebarItemProps> = ({ item, path = [] }) => {
  ...
  return (
    <li>
      <Link href={href}
        className={`flex items-center p-2 text-gray-300 rounded-lg hover:bg-gray-700 group ${hasChildren ? 'font-bold' : 'text-sm'}`}
        onClick={(e) => {
          if (hasChildren) {
            e.preventDefault();
            setIsOpen(!isOpen);
...
const Sidebar: React.FC<SidebarProps> = ({ data }) => {
  return (
    <aside id="logo-sidebar" className="fixed bg-[hsl(210,3%,23%)] top-0 left-0 z-40 w-80 h-screen pt-20 transition-transform -translate-x-full border-r border-gray-700 lg:translate-x-0" aria-label="Sidebar">
